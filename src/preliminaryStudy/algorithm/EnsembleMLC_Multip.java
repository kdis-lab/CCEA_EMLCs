 ...
 *
 */
public class EnsembleMLC_Multip extends MultiLabelMetaLearner {
	 ...
	 */
	private List<MultipBinArrayIndividual> EnsembleInds;
	 ...
	 */
	public EnsembleMLC_Multip(List<MultipBinArrayIndividual> EnsembleInds, MultiLabelLearner baseLearner, int numClassifiers, Hashtable<String, MultiLabelLearner> tableClassifiers)
	{
		super(baseLearner);
	 ...
	 */
	public List<MultipBinArrayIndividual> getEnsembleInds()
	{
		return EnsembleInds;
	}
	...
	public byte[][] getEnsembleMatrix(){
		...
		for(int i=0; i<numClassifiers;i++) {
			mat[i] = EnsembleInds.get(i).getGenotype();
		}
	...
	public String toString()
	{
		...
		{	
			str += EnsembleInds.get(model).toString();
			str+="\n";
		}
		return str;
	}
	...
	protected void buildInternal(MultiLabelInstances trainingData)
			...
			for(int i = 0; i < numClassifiers; i++)
			{
				...
				if(!tableClassifiers.containsKey(s)) {
					System.out.println("table of size " + tableClassifiers.size() + " doesnt have ind " + s);
				}
